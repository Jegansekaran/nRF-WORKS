...
float final_value(void)
{
    ...
    if (hex_value()){
        ...
        char tx_buf[64];
        snprintf(tx_buf, sizeof(tx_buf), "Measurement error");
                         ...
                         uint16_t len)
{
    ...
    if ((val >> 28) == 0x0) {
        ...
        LOG_INF("tare freq =%f K = %f",tare,K);
    }
}
...
void calibration_thread(void)
{
    ...
    while(1){
        ...
        //LOG_INF("Tare complete freq: %.2f || k: %f\n", tare, K);
    }
...
void send_thread(void)
{
    ...
    while (1) {
        ...
        if(atomic_get(&freq_received)){
            snprintf(tx_buf, sizeof(tx_buf), "Freq: %.2f Hz\r\n", value);
            atomic_set(&freq_received, 0);
            }
...
int main(void)
{
    ...
    vm511_write_reg(0x0A,0x000D);
    k_msleep(3);
    vm511_write_reg(0x0F,300);
    k_msleep(3);
    vm511_write_reg(0x01,8000);
    k_msleep(3);    
