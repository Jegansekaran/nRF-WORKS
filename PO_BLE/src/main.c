...
static const struct gpio_dt_spec vcc_pin = GPIO_DT_SPEC_GET(VCC, gpios);
...
{   
    ...
    return (int32_t)((raw - offset) / scale);
...
bool calibrate(int32_t known_weight)
{
    ...
    if (abs(delta) < 10000)
    {
        //LOG_ERR("Calibration failed: insufficient delta");
        return false;
    }

    scale = (float)delta / (float)known_weight;
    //LOG_INF("Offset: %d, Scale: %f", offset, scale);
    return true;
}
        ...
        if(atomic_get(&cali_done)) {
            ...
            if (!load_calibration())
            {
                ...
                if (calibrate(set_load)){
                       ...
                       gpio_pin_set_dt(&sck_pin, 0); 
                       k_msleep(100);
                       gpio_pin_configure_dt(&sck_pin, GPIO_INPUT);
	                   gpio_pin_set_dt(&vcc_pin, 0);
...
int main(void)
{
    ...
    gpio_pin_configure_dt(&dt_pin, GPIO_INPUT);
    gpio_pin_configure_dt(&sck_pin, GPIO_INPUT);
	gpio_pin_configure_dt(&vcc_pin, GPIO_OUTPUT_LOW);
	//gpio_pin_set_dt(&vcc_pin, 1);
